...
const char *get_section_use(char *name)
{
...
const char *get_section_flags(MAX_BYTES sec_flags)
{
...
void print_sections()
{
	...
	for (i=0; i<GET_BYTES(fptr->ehdr.e_shnum); i++) {
		...
		const char *sec_type 	= get_section_type(GET_BYTES(fptr->shdr[i].sh_type));
