...
#define DYNAMIC_SECTION_STRUCT_ALLOCATE_DVAL(i) \
		{ \
			ALLOCATE(dsec[i].d_tag); \
			ALLOCATE(dsec[i].d_un.d_val); \
		}

#define DYNAMIC_SECTION_STRUCT_ALLOCATE_DPTR(i) \
		{ \
			ALLOCATE(dsec[i].d_tag); \
			ALLOCATE(dsec[i].d_un.d_ptr); \
		}
...
void init_32_bit_dynamic_section_struct(MAX_BYTES num_dsection)
{
	...
	for (i=0; i<num_dsection; i++) {
		if (sizeof(dsec[i].d_un.d_val) > sizeof(dsec[i].d_un.d_ptr)) {
			DYNAMIC_SECTION_STRUCT_ALLOCATE_DVAL(i);
		} else {
			DYNAMIC_SECTION_STRUCT_ALLOCATE_DPTR(i);
		}
	}
	free(dsec);
}
...
const char *get_lib_name(MAX_BYTES d_tag, MAX_BYTES d_val, const char *d_type)
{
...
void print_dynamic_section()
{
	...
	for (i=0; i<fptr->num_dsec; i++) {
		...
		const char *d_type 	= get_dynamic_type(d_tag);
