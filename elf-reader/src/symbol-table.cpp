...
void print_symbol_table()
{
	...
	if (fptr->num_sym) {
		...
		for (i=0; i<fptr->num_sym; i++) {
			...
			const char *type = get_symbol_type(ELF_ST_TYPE (GET_BYTES(fptr->sym[i].st_info)));
			const char *bind = get_symbol_binding(ELF_ST_BIND (GET_BYTES(fptr->sym[i].st_info)));
			const char *visb = get_symbol_visibility(ELF_ST_VISIBILITY (GET_BYTES(fptr->sym[i].st_info)));
			const char *sndx = get_symbol_index_type(GET_BYTES(fptr->sym[i].st_shndx));
